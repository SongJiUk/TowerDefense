...
public class UI_NextWavePanel : UI_Base
{
    ...
    enum Images { Image_Wave, Image_StartEarly_BG, Image_StartEarly_Border }
    enum GameObjects
    {
    ...
    public void SetInfo(WavePreview preview)
    {
        ...
        string spriteName = isBoss ? "wave_boss" : (isMidBoss ? "wave_middleboss" : "wave_normal");
        GetImage(typeof(Images), (int)Images.Image_Wave).sprite = Managers.ResourceM.GetAtlas(spriteName);
    ...
    {
        GetText(typeof(Texts), (int)Texts.Text_WaveNumber).color = text;
